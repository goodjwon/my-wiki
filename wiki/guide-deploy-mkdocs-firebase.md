...
```mermaid
flowchart LR
  A[wiki/*.md<br/>Obsidian 편집] --> B[빌드 스크립트<br/>wiki/ → docs/ 복사]
  B --> C[mkdocs build]
  C --> D[site/ 정적 파일]
  D --> E[firebase deploy]
  E --> F[Firebase CDN<br/>전 세계 배포]
  F --> G((<project>.web.app<br/>또는 커스텀 도메인))
...
```mermaid
flowchart TD
  A[Obsidian에서 wiki/ 편집] --> B{공개 페이지?}
  B -->|예| C[./scripts/deploy.sh]
  B -->|아니오| D[git commit만]
  C --> E[Firebase CDN 갱신]
  C --> F[git commit & push]
  D --> F
