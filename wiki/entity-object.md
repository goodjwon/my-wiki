...
한국어로 쓰인 객체지향 설계 입문서 중 가장 자주 권장되는 책입니다. **"책임 주도 설계(Responsibility-Driven Design)"** 관점에서 OO를 처음부터 다시 짭니다 — 데이터·상속·다형성 같은 문법 항목이 아니라 **객체 간의 협력과 책임 분배**를 중심에 두고 절·장이 흘러갑니다.
...
이 9개가 책 전체를 관통하는 골격입니다. 나머지 챕터는 모두 이 메시지를 영화 예매·핸드폰 과금 두 시그니처 예제로 반복 증명합니다.

## 챕터 흐름 (4부 구조)

목차에는 부 구분이 없지만 흐름상 4부로 묶을 수 있습니다.
...
OO 설계 핵심 5권이 서로 다른 단위·시점에서 같은 OO 결론을 가리킵니다.
...
> ***오브젝트* 의 자리** : OO 설계의 **목적지** 를 가장 풍부한 한국어 시나리오 (영화 예매·핸드폰 과금) 로 그려냅니다. 다른 4권이 매번·1단계·1줄·1사이클 단위라면 *오브젝트* 는 **객체·협력 단위의 큰 그림**입니다.

**추천 학습 순서** (5권 전체):

1. *Clean Code* 2·3·4·10장 — 가장 빠른 효과 (줄·이름)
2. *Effective Java* 2·3·8·10장 — Java 권고 90 개
3. *리팩터링* 3장 (24 악취) + 6장 (기본) — 기존 코드 개선 어휘
4. ***오브젝트* 1~5장 + 11장** — OO 설계 큰 그림 (이 책)
...
→ 두 시그니처 예제(영화 예매·핸드폰 과금)는 책을 통째로 관통하므로, 한 번 익혀두면 인용·발표 자리에서 바로 꺼내 쓸 수 있는 자산이 됩니다.
...
→ **공통 원리**: 정적 구조(데이터·필드·기본값)에서 출발하면 동적 행동(변경·부하·진화)을 못 견딥니다. **행동·시나리오부터 정의한 뒤 정적 구조를 유도**하는 것이 변경에 강한 설계의 공통 원칙입니다.
...
위 중 2개 이상이면 1~5장 + 11장만 먼저 읽어도 가치 있습니다.

## 한계·주의

- **분량이 크다** (700페이지) — 절·장이 반복적이라 한 번에 다 읽기는 어렵습니다. **3장·5장·11장·13장**이 가장 응축돼 있어 그것부터 발췌해 읽어도 됩니다.
- **한국어 전용** — 영어판이 없습니다. 해외 동료와 같은 책으로 토론하기는 어렵습니다.
- **Java 7~8 기준 코드** (2019년 출판) — record·sealed 같은 신문법은 다루지 않습니다. 메시지·책임 관점은 언어 버전과 무관해서 큰 문제는 아닙니다.
