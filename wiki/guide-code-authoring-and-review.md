...
**코드 작성 + 코드 점검** 두 워크플로를 한 페이지로 압축한 가이드입니다. 함수·객체·의존성·예외·테스트·동시성의 실무 체크리스트와 PR 리뷰 어휘를 담았습니다.
...
> **실습 환경 먼저** — Java 17 + JUnit 5 + Python + Node 셋업은 [[guide-java-book-study-lab]] 을 참조합니다. 이 가이드는 환경이 준비된 후의 **작성·점검 워크플로**를 다룹니다.

> 💡 이 가이드는 OO 설계 도서 5권의 결론을 한 워크플로로 압축한 것입니다. 각 항목의 근거 도서·장은 본문 캡션과 표에 표시했고, 책별 요약은 맨 아래 **7. 참고** 섹션에 있습니다.
...
→ 거의 모든 악취가 **6장(기본)·7장(캡슐화)·8장(이동)** 카탈로그로 해결됩니다. 한 코드에 여러 악취가 겹치는 게 보통이므로 가장 큰 것부터 처리합니다.
...
→ **정보 전문가**가 출발점입니다. 단 정보 객체가 비대해지면 **높은 응집도**와 균형을 잡습니다. 9패턴은 외울 규칙이 아니라 사고 도구입니다.
...
> **명령의 작동 원리** — 두 명령은 이 가이드 본문을 베이스로 동작합니다. my-wiki 프로젝트 안에선 `wiki/` 파일을 직접 읽고, **다른 프로젝트에선 `wiki/` 가 없으므로 자동으로 `https://wiki.wonslab.dev/...` 를 WebFetch** 합니다(그때만 인터넷 필요). 그래서 명령 파일만 있으면 어디서든 동작합니다.
...
다른 프로젝트(회사·사이드)에서도 두 명령을 쓰려면 명령 파일만 옮기면 됩니다.

**방법 A — 명령 파일 전체를 복붙 (가장 확실·오프라인 OK) ⭐**

아래 두 코드블록을 그대로 복사해 새 프로젝트에 저장하면 끝납니다. my-wiki 클론도 별도 설치도 필요 없습니다.
...
저장 후 Claude Code 를 다시 열면 사용할 수 있습니다.

> ⚠️ 위 두 블록은 **스냅샷**입니다. 원본은 GitHub 에 있습니다(방법 B). 명령을 고쳤다면 이 블록도 함께 갱신해야 어긋나지 않습니다.
...
이쪽이 **원본**이므로 항상 최신입니다(방법 A 블록보다 우선). 단 GitHub 접근이 필요합니다.

### 4.3 다른 AI(ChatGPT·Cursor·Gemini)에서 — 프롬프트·본문

슬래시 명령이 없는 도구에선 (a) 이 페이지 본문을 첫 메시지 컨텍스트로 주거나 (b) 아래 프롬프트 템플릿을 복붙합니다.
...
→ 코멘트 한 줄이 책 한 권을 인용하는 셈이므로 **합의가 빨라집니다**.

### 4.6 신입 교육

한 페이지로 핵심 점검 어휘를 익히게 합니다. 인턴·신입에게는 이렇게 안내합니다.
...
→ **공통 원리**: 안전한 시스템은 **이름·전제·절차가 명시된 메커니즘** 의 누적으로 만들어집니다. "잘 알아서 한다" 는 가정은 어디서나 위험합니다.
