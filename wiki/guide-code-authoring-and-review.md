...
| 장 | Item | 적용 시점 |
|----|------|----------|
| 2 객체 생성·파괴 | 1 | `new` 대신 `of`·`from`·`valueOf` 정적 팩터리 |
| 2 객체 생성·파괴 | 5 | 의존성 주입 (생성자 주입) |
| 2 객체 생성·파괴 | 9 | try-finally 대신 try-with-resources |
| 3 공통 메서드 | 11 | equals 재정의 시 hashCode 도 재정의 |
| 4 클래스·인터페이스 | 17 | 불변 클래스 우선 (record 활용) |
| 4 클래스·인터페이스 | 18 | 상속보다 컴포지션 |
| 4 클래스·인터페이스 | 20 | 추상 클래스보다 인터페이스 |
| 5 제네릭 | 31 | PECS — Producer Extends, Consumer Super |
| 6 열거·애너테이션 | 34 | int 상수 대신 enum |
| 6 열거·애너테이션 | 39 | 명명 패턴 대신 애너테이션 |
| 7 람다·스트림 | 45 | 스트림은 신중히 (가독성 1순위) |
| 7 람다·스트림 | 48 | 스트림 병렬화는 측정 후 |
| 8 메서드 | 49 | public 메서드 첫 줄에 매개변수 검증 |
| 8 메서드 | 50 | 적시에 가변 객체 방어적 복사 |
| 9 일반 프로그래밍 | 64 | 객체는 인터페이스 타입으로 참조 |
| 9 일반 프로그래밍 | 67 | 최적화는 신중히 (측정 후) |
| 10 예외 | 77 | 예외를 무시하지 말 것 |
| 11 동시성 | 78 | 공유 가변 데이터는 동기화 |
| 11 동시성 | 79 | 과도한 동기화 회피 (락 안에서 외부 콜백 X) |
| 12 직렬화 | 85 | 자바 직렬화보다 대안 우선 |
