...
이 트랙은 [[src-java-study-2024-2025]] 10장 재구성. 원문: `raw/java-study/java-study-ch10-입출력과네트워크.md`.
...
| 순서 | 문서 | 한 줄 |
|------|------|------|
| 10.0 | 입출력과 네트워크 개요 | 바깥 세계와의 경계는 늘 실패 가능 |
| 10.1 | Java 예외 종류 정리 | Checked vs Unchecked, 언제 무엇을 |
| 10.2 | 예외와 사용자 정의 예외 | 의미 있는 예외로 도메인 표현 |
| 10.3 / 10.4 | 파일 읽기·쓰기, 여러 형식 처리 | try-with-resources로 자원 닫기 |
| 10.5 | 네트워크 프로그래밍 기초 | 소켓, 클라이언트-서버 |
| 10.6 | JDBC 기초 | DB와 직접 대화 (T4에서 추상화) |
| 10.7 | 파일 처리 설계 실습 | 🛠 워밍업 |
| 10.8 / 10.9 | 파일 처리 실전문제 | 🛠 아래 미니프로젝트 |
...
> 10.5 소켓을 배우면 "왜 Keep-Alive·멀티플렉싱·QUIC이 나왔나"가 보인다. HOL 블로킹이 그 출발점.
...
1. **파일 처리 실전문제 1 (10.8)** — 텍스트 파일 읽어 가공·저장. 예외·자원 닫기를 정석으로.
2. **형식별 파일 처리 (10.9)** — CSV·JSON 등 형식별 분기를 전략 패턴(T2)으로 깔끔하게.
...
```
원문 풀이: `raw/java-study/java-study-ch10-입출력과네트워크.md`(10.8-1·10.8-2·10.9-1), `java-study-ch11-부록.md`(11.9).
