...
<details><summary>Q. <code>public boolean equals(PhoneNumber pn)</code>로 작성하면 무엇이 잘못되는가?</summary>

`Object`를 받지 않으므로 **재정의(override)가 아니라 다중정의(overload)**가 됩니다. `Object` 참조로 호출하면 `Object.equals`(참조 비교)가 불려서 의도와 다르게 동작합니다. `@Override`를 붙였다면 컴파일 에러로 잡혔을 것입니다.
...
<details><summary>Q. hashCode를 항상 같은 상수로 반환하면 정상 동작은 하는데 왜 권장하지 않는가?</summary>

equals/hashCode **규약은 지키므로 오작동은 없지만**, 모든 객체가 같은 버킷에 들어가 해시 테이블이 **연결 리스트처럼 O(n)으로 퇴화**해 성능이 무너집니다. 비즈니스 키가 있으면 그것을 쓰는 편이 좋습니다.
...
<details><summary>Q. 엔티티의 toString에서 양방향 연관 필드를 빼야 하는 이유는?</summary>

서로의 toString을 호출하며 **무한 재귀(StackOverflowError)**가 발생하고, 지연 로딩 필드를 강제로 초기화하는 부작용도 있기 때문입니다.
...
<details><summary>Q. clone 대신 복사 생성자/팩터리가 나은 점을 두 가지 들어라.</summary>

①정상적인 생성자를 거치므로 `final` 필드·불변식·검증과 충돌하지 않는다. ②인터페이스 타입을 인자로 받아 **다른 구현으로의 변환 복사**(예: List→ArrayList)가 가능하다.
...
<details><summary>Q. <code>o1.value - o2.value</code> 방식의 비교가 위험한 이유는?</summary>

값의 차이가 `int` 범위를 넘으면 **오버플로**로 부호가 뒤집혀 잘못된 순서가 나옵니다. 부동소수에서도 정밀도 문제가 생깁니다. `Integer.compare`/`Double.compare`를 써야 안전합니다.
...
<details><summary>Q1. "값 객체"와 "엔티티"의 equals 전략은 어떻게 다른가?</summary>

값 객체는 **모든(또는 핵심) 필드 값**이 같으면 같다고 본다. 엔티티는 **정체성(불변 비즈니스 키 또는 id)** 기준으로 같음을 판단하며, 변하는 필드나 나중에 채워지는 id를 hashCode에 직접 넣지 않는다.

</details> <details><summary>Q2. equals만 재정의하고 hashCode를 빠뜨리면 가장 먼저 어디서 터지는가?</summary>

`HashMap`/`HashSet` 같은 **해시 기반 컬렉션**에서 넣은 객체를 못 찾거나(get=null), 중복이 제대로 제거되지 않습니다.

</details> <details><summary>Q3. toString 재정의 시 현업에서 가장 조심할 한 가지는?</summary>

**민감정보 노출.** 비밀번호·주민번호 등이 로그/예외 메시지로 새어 나가지 않도록 제외·마스킹해야 합니다(개인정보·ISMS-P 위반 방지).
