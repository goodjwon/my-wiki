...
> **따라 하는 법**: 위에서 아래로 읽으며 코드를 직접 쳐본다. 설치 → Hello World → **1.2에서 Maven/Gradle로 진짜 프로젝트를 만들어 빌드·실행**까지 명령으로 한 바퀴 돌린다.
...
#### 독자가 먼저 맞춰야 할 기준
- 두 실습 프로젝트를 따라가려면 Java 21 환경이 가장 안전합니다.
- 예제 대부분은 Java 17에서도 동작하지만, 문서를 읽는 기준선은 우선 Java 21로 맞추는 편이 흐름이 덜 흔들립니다.
...
가장 단순한 출발점은 콘솔에 한 줄을 출력하는 프로그램입니다. 아직 프로젝트를 만들기 전 단계이므로, 아무 작업 디렉터리(예: `~/hello-first`)에 **파일 하나**만 만들어 시작합니다. package 선언도 아직 필요 없습니다(프로젝트 구조로 옮기는 것은 1.2에서 다룹니다).
...
> **Mac·Linux ↔ Windows** — 명령은 대부분 공통이고, 세 가지만 다릅니다:
>
> | | Mac · Linux | Windows |
> |---|---|---|
...
두 도구 모두 아래 표준 레이아웃을 만듭니다 (빌드 파일만 다름):
...
> 처음엔 `run`/`java -cp`로 콘솔에 "Hello" 한 줄이 찍히는 것만 확인하면 됩니다. 중요한 건 **"소스 → 빌드 → 실행" 사이클을 도구로 한 바퀴 실행해 보는 것**입니다.
...
빌드 산출물은 커밋하지 않습니다. 먼저 저장소를 초기화하고:

```bash
git init
```

**편집기/IDE로** 프로젝트 루트에 `.gitignore` 파일을 만들어 아래 내용을 넣습니다 (gitignore 주석은 줄 전체 `#`만 인식하므로 값 옆에 쓰지 않습니다):
...
- 패키지 디렉터리 경로와 `package` 선언을 다르게 두는 것
- IDE에서만 실행해 보고 `mvn`/`gradlew` 명령 빌드는 실행해 보지 않는 것
- 의존성을 손으로 jar 받아 넣는 것 (빌드 파일에 좌표로 선언해야 함)

### ✏️ 첫 프로젝트 만들기 (Maven·Gradle) 직접 해보기
...
Java는 운영체제와 독립적인 플랫폼 독립성을 제공하므로 다양한 환경에서 실행할 수 있습니다.

#### 1.1 Java의 플랫폼 독립성
- **JVM(Java Virtual Machine)**: Java 프로그램은 JVM에서 실행되며, JVM은 각 운영체제에 맞게 구현되어 있습니다.
- **바이트코드**: Java 컴파일러는 소스 코드를 바이트코드(.class 파일)로 컴파일하고, JVM은 바이트코드를 해석하여 실행합니다.
- **Write Once, Run Anywhere**: 한 번 작성한 코드를 다양한 운영체제에서 실행할 수 있다는 의미입니다.

#### 1.2 운영체제와의 상호 작용
- Java는 운영체제의 세부 사항을 추상화하여 개발자가 직접 운영체제에 종속적인 코드를 작성하지 않도록 합니다.
- **JNI(Java Native Interface)**: 필요한 경우 C나 C++로 작성된 네이티브 코드를 호출하여 운영체제의 특정 기능을 활용할 수 있습니다.

### 2. Java와 C/C++의 비교
Java와 C/C++은 모두 프로그래밍 언어지만, 여러 면에서 차이점이 존재합니다.

#### 2.1 언어의 특징 비교

| 특징 | Java | C/C++ |
|------|------|-------|
| 컴파일 과정 | 바이트코드로 컴파일 후 JVM에서 실행 | 기계어로 컴파일되어 직접 실행 |
| 플랫폼 독립성 | 높음 (JVM 덕분에) | 낮음 (운영체제에 따라 재컴파일 필요) |
| 메모리 관리 | 자동 가비지 컬렉션으로 메모리 관리 | 개발자가 직접 메모리 관리 (malloc, free 등) |
| 포인터 지원 | 직접적인 포인터 사용 불가 | 포인터를 통해 메모리 직접 조작 가능 |
| 프로그램 속도 | 기동·초기엔 느리나 JIT 컴파일 후 격차 축소 | 빠름 (기계어로 직접 실행) |
| 멀티스레딩 지원 | 언어 수준에서 지원 | C++11부터 표준 라이브러리(std::thread) 지원, C는 OS API(pthread 등) 사용 |
| 객체 지향 특징 | 객체 지향 중심 (단, int 등 기본형은 객체가 아님) | C는 절차적 언어, C++은 객체 지향과 절차적 프로그래밍 지원 |

#### 2.2 메모리 관리 차이
- **Java**
  - 자동 가비지 컬렉션으로 메모리 누수 위험을 줄입니다.
  - 개발자는 메모리 해제에 신경 쓸 필요가 없습니다.
- **C/C++**
  - 동적 메모리 할당 시 `malloc`, `free` 등을 사용하여 직접 관리합니다.
  - 메모리 누수 및 잘못된 메모리 접근의 위험이 있습니다.

#### 2.3 실행 환경 차이
- **Java**
  - JVM 위에서 바이트코드를 해석하여 실행합니다.
  - JVM의 최적화(JIT 컴파일러 등)로 성능을 향상시킬 수 있습니다.
- **C/C++**
  - 컴파일 시 기계어로 변환되어 운영체제에서 직접 실행합니다.
  - 하드웨어 자원을 직접 제어할 수 있어 고성능 프로그램 개발에 유리합니다.

#### 2.4 사용 분야
- **Java**
  - 웹 애플리케이션, 안드로이드 앱, 기업용 소프트웨어 등에 사용됩니다.
  - 플랫폼 독립성과 풍부한 라이브러리로 다양한 분야에서 사용됩니다.
- **C/C++**
  - 시스템 프로그래밍, 게임 개발, 임베디드 시스템 등에 사용됩니다.
  - 하드웨어와 밀접한 고성능 프로그램 개발에 적합합니다.
...
### 3. 결론
- Java는 플랫폼 독립성과 자동 메모리 관리 등의 장점을 가지며, 운영체제에 종속되지 않는 응용 프로그램 개발에 유리합니다.
- C/C++은 하드웨어 자원에 대한 직접적인 접근과 높은 성능을 제공하여 시스템 프로그래밍 등에 적합합니다.
- 개발 목적과 요구 사항에 따라 적합한 프로그래밍 언어를 선택하는 것이 중요합니다.
