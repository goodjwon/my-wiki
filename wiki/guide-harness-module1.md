...
**프롬프트 머리의 `[베이스라인 측정 중 …]` 표시**: 여러분이 Claude에게 보내는 프롬프트의 첫 줄이고, 이미 아래 프롬프트에 포함돼 있으므로 **그대로 붙여넣기만 하면 됩니다 — 따로 할 일은 없습니다**. 주어를 살려 풀면 "**나는(사용자) 지금 너의(Claude) 기본 성능을 재는 중이다** — 규칙(CLAUDE.md·hooks)은 없으니 평소 하던 대로 작업해라"라는 뜻입니다. 측정하는 쪽은 여러분이고, 측정당하는 쪽이 Claude입니다. 태스크 B·C의 `[베이스라인 측정 중]`은 같은 표시의 줄임입니다. Module 02에서 같은 태스크를 다시 시킬 때는 이 줄만 빼고 보내서, 전후 차이로 하네스 효과를 확인합니다.
