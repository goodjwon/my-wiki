...
**프롬프트 머리의 `[베이스라인 측정 중 …]` 표시**: 프롬프팅 관점에서 의미가 있는 한 줄입니다. ① "CLAUDE.md, hooks 없음"은 규칙 문서가 없는 것이 실수가 아니라 의도임을 미리 알리는 컨텍스트 제공이라, Claude가 규칙을 찾아 헤매거나 되묻지 않고 바로 작업합니다. ② "평소처럼 작업해줘"는 측정을 위해 평소 습관 그대로 움직이게 하는 행동 지시입니다. 태스크 B·C 프롬프트의 `[베이스라인 측정 중]`은 같은 표시의 줄임입니다. Module 02에서 같은 태스크를 재실행할 때는 측정이 아니라 실전 적용이므로 이 표시를 뗍니다.
