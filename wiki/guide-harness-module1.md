...
**왜 측정부터 시작하나**: 하네스는 "에이전트의 반복 실수를 시스템으로 막는 장치"입니다. 그런데 무엇을 막을지 모르면 규칙을 만들 수 없고, 적용 전 상태를 기록해 두지 않으면 나중에 하네스가 효과 있었는지 증명할 수 없습니다. 그래서 첫 모듈은 규칙 작성이 아니라 **측정과 기록**입니다. 여기서 측정이란 계측 도구가 아니라, Claude에게 일을 시켜 놓고 결과물을 몇 가지 항목(테스트를 썼는가, 안 시킨 코드를 몇 줄 넣었는가, 메시지를 몇 번 주고받았는가)으로 세어 손으로 적는 일입니다.

**따라하는 입장에서 이 모듈의 의미**: 시험공부로 치면 인강부터 듣는 게 아니라 **모의고사를 먼저 쳐서 틀린 문제를 확보하는 단계**입니다. 규칙 없이 시켜 보면 에이전트의 기본 습관(테스트 생략, 안 시킨 리팩터링, 내부 모델 노출)을 직접 목격하게 되는데, 그 목격 기록이 ① Module 02에서 만들 규칙의 재료가 되고 ② Module 05에서 "내가 만든 하네스가 실제로 효과 있었다"를 확인할 비교 기준이 됩니다. 이 단계를 건너뛰면 Module 02에서 남의 규칙을 베끼게 되고, 마지막에 개선을 증명할 방법도 없어집니다.
...
**목적**: 하네스 적용 **전** 에이전트 성능을 수치로 기록합니다. Module 02(태스크 D — address 필드)와 Module 05(태스크 E — birthDate 필드)에서 **같은 유형의 새 태스크**를 시켜 After와 비교합니다 — 같은 태스크를 그대로 재실행하면 "이미 구현돼 있다"로 끝나기 때문에, 요구 구조와 측정 항목만 고정하고 필드를 바꿉니다.
...
**태스크가 왜 이 세 개인가**: 실무에서 에이전트에게 가장 자주 시키는 작업 유형 3가지 — 기능 추가(A), 기존 API 변경(B), 버그 수정(C) — 를 하나씩 골랐습니다. 유형마다 드러나는 나쁜 습관이 다르기 때문에, 세 개를 다 돌려야 Module 02에서 만들 규칙의 재료가 고르게 모입니다.

**프롬프트 머리의 `[베이스라인 측정 중 …]` 표시**: 프롬프팅 관점에서 의미가 있는 한 줄입니다. ① "CLAUDE.md, hooks 없음"은 규칙 문서가 없는 것이 실수가 아니라 의도임을 미리 알리는 컨텍스트 제공이라, Claude가 규칙을 찾아 헤매거나 되묻지 않고 바로 작업합니다. ② "평소처럼 작업해줘"는 측정을 위해 평소 습관 그대로 움직이게 하는 행동 지시입니다. 태스크 B·C 프롬프트의 `[베이스라인 측정 중]`은 같은 표시의 줄임입니다. Module 02에서 같은 유형의 새 태스크(태스크 D)를 실행할 때는 측정이 아니라 실전 적용이므로 이 표시를 뗍니다.

### 태스크 A — 모델에 필드 추가 (10분)

모델→API→화면을 관통하는 필드 추가는 가장 흔한 기능 추가 작업입니다. 내부 모델을 응답에 그대로 노출하는지, 입력 검증을 어떤 방식으로 하는지, 시키지 않아도 테스트를 쓰는지가 여기서 드러납니다.

Claude Code(playground 루트)에서 코드 블록 오른쪽 위 복사 버튼으로 아래를 **통째로 복사해 붙여넣습니다**:
...
이번에는 이미 있는 API의 응답 형태를 바꾸는 작업입니다. 배열을 객체로 바꾸면 기존 사용처가 깨지는데, 에이전트가 web/ 쪽 하위 호환을 스스로 챙기는지, 요청하지 않은 기능(정렬·필터)까지 과잉 구현하는지, 경계값을 처리하는지가 드러납니다.

역시 통째로 붙여넣습니다 (첫 줄은 태스크 A와 같은 측정 표시의 줄임):
...
마지막은 버그 수정입니다. 새 코드를 짜는 능력이 아니라 일하는 순서를 봅니다 — 고치기 전에 재현 테스트부터 쓰는지, 원인을 가정으로 말하고 시작하는지, 관련 없는 코드까지 손대는지가 드러납니다.

역시 통째로 붙여넣습니다 (첫 줄은 같은 측정 표시의 줄임):
...
측정은 기록으로 남겨야 비교할 수 있습니다. Step 2의 감사 표와 Step 3의 측정 시트를 `.claude/baseline.md` 한 파일로 합칩니다 — Module 05에서 이 파일을 열어 놓고 같은 유형의 새 태스크(태스크 E)를 실행하며 Before/After를 대조하게 됩니다.
...
> 이 파일의 수치는 Module 02(태스크 D — address)·Module 05(태스크 E — birthDate)에서
> 같은 유형의 새 태스크를 실행해 After와 비교합니다. 태스크 본문이 아니라
> 요구 구조(필드 추가·형식 검증·필수 처리·web 반영·테스트)와 측정 항목을
> 고정해야 비교가 유효합니다.
EOF
