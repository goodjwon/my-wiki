...
**프롬프트 머리의 `[베이스라인 측정 중 …]` 표시**: "지금은 규칙 없이 평소 습관대로 작업하라"고 Claude에게 알리는 안내 줄입니다. 태스크 B·C의 `[베이스라인 측정 중]`은 같은 표시의 줄임입니다. Module 02에서는 이 표시를 떼고 CLAUDE.md가 걸린 상태로 같은 태스크를 다시 던져, 그 차이로 하네스 효과를 확인합니다.
