...
**Q1. 다형성으로 바꾸기(10.4)가 책의 ★인 이유?**

**A.** OO의 핵심을 가장 직접적으로 보여주는 리팩터링. switch는 새 타입마다 모든 분기 찾아 추가 (산탄총 수술), 다형성은 새 타입 = 새 클래스 1개로 끝 (OCP). 객체지향이 "왜 좋은가" 의 가장 구체적 사례.

**Q2. "단일 반환점" 전통 권고와 보호 구문(10.3)이 충돌하는데, 왜 보호 구문이 더 권장되나?**

**A.** 단일 반환점은 옛 C 시대의 자원 정리 패턴에서 왔지만, 현대 언어는 GC + try-with-resources로 그 문제가 사라짐. 보호 구문이 정상 경로의 의도를 가장 명료하게 드러내므로 가독성에서 압도적.

**Q3. Java `assert`가 public API 검증에 부적합한 이유?**

**A.** 기본 비활성 (`-ea` JVM 옵션이 있어야만 작동). 운영 환경에서 비활성이면 검증 안 됨. public 메서드는 항상 검증돼야 하므로 `Objects.requireNonNull` 같은 명시적 throw 필요. assert는 private/디버깅용.

**Q4. Null Object 패턴(10.5)을 항상 도입하지 말라는 이유?**

**A.** null 체크가 1~2곳뿐이면 패턴 도입 비용이 더 큼 — 새 클래스, 팩터리, equals 처리 등. **여러 곳에서 같은 null 처리가 반복** 될 때만 가성비 있음.
