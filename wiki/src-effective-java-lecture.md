...
| 장 | 주제 | 아이템 범위 | raw 파일 | ⭐ 현업 최핵심 |
|----|------|------------|----------|----------------|
| **2장** | 객체 생성과 파괴 | Item 1~9 | `raw/effective_java/이펙티브 자바 실전 강의 교재 2장.md` | **5**(DI), **9**(try-with-resources) |
| **3장** | 모든 객체의 공통 메서드 | Item 10~14 | `...3장.md` | **11**(equals+hashCode) |
| **4장** | 클래스와 인터페이스 | Item 15~25 | `...4장.md` | **17**(불변), **18**(컴포지션), **20**(인터페이스) |
| **5장** | 제네릭 | Item 26~33 | `...5장.md` | **31**(PECS) |
| **6장** | 열거 타입과 애너테이션 | Item 34~41 | `...6 장.md` | **34**(enum), **39**(애너테이션) |
| **7장** | 람다와 스트림 | Item 42~48 | `...7장.md` | **42**(람다), **45**(스트림 신중), **48**(병렬화 주의) |
| **8장** | 메서드 | Item 49~56 | `...8장.md` | **49**(매개변수 검증), **50**(방어적 복사) |
| **9장** | 일반적인 프로그래밍 원칙 | Item 57~68 | `...9장.md` | **59**(라이브러리), **60**(BigDecimal), **64**(인터페이스 참조), **67**(최적화) |
| **10장** | 예외 | Item 69~77 | `...10장.md` | **73**(추상화 수준), **77**(예외 무시 금지) |
| **11장** | 동시성 | Item 78~84 | `...11장.md` | **78**(동기화), **79**(과도 동기화 회피) |
| **12장** | 직렬화 | Item 85~90 | `...12장.md` | **85**(자바 직렬화 대안), **90**(직렬화 프록시) |

→ 총 **90 아이템**, **5,200+ 줄**, **⭐ 20개 현업 최핵심**.
