...
- 가정을 명시적으로 선언합니다. 불확실하면 먼저 질문합니다.
- 더 단순한 방법이 있으면 먼저 말하고, 필요할 때 push back합니다.
- 여러 해석이 가능하면 조용히 하나를 고르지 말고 옵션을 제시합니다.

### 2. Simplicity First (단순함 우선)
- 요청된 것만 구현합니다. "나중에 쓸 수도 있는" 메서드 추가는 금지합니다.
- 100줄이 30줄로 가능하면 다시 씁니다.
- "시니어 엔지니어가 보면 과하다고 할까?" → Yes면 단순화합니다.

### 3. Surgical Changes (외과적 변경)
- 기존 네이밍/패키지를 "더 나은 방향"으로 임의 리팩토링은 금지합니다.
- 내 변경으로 생긴 unused import/변수만 정리합니다.
- 기존 dead code는 **언급만 하고 삭제하지 않습니다**.
- 모든 변경된 줄은 사용자 요청에서 추적 가능해야 합니다.
...
에이전트가 다음 행동을 하려 할 때 **즉시 멈추고 사용자에게 확인을 요청합니다**:
...
이 STOP 트리거 중 일부는 [[concept-claude-hooks]]의 guard.sh로 **물리적으로 차단**할 수 있습니다 (예: main 브랜치 직접 push, migration 삭제).

## 누적된 실패 패턴 섹션

CLAUDE.md 하단에는 반드시 다음 표를 둡니다 — 에이전트가 같은 실수를 반복하면 추가:
