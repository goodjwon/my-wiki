...
> ✅ **실행 검증됨 (2026-07-12, Node v24, Claude Code 2.1.197)**: Step 0~장면 4를 실제로 돌려 확인했습니다 (장면 세션은 헤드리스 `claude --agent advisor -p` + stream-json 도구 호출 감사). ① 장면 1 — 위임 전 `node cart.test.js`로 실패 확인 후, 실패 케이스(기대 40000·실제 45000)가 담긴 브리프로 worker 위임, cart.js Edit는 Worker만, Advisor가 git diff+재테스트 후 커밋. 단 오버헤드 예외를 "한두 줄" **크기 기준**으로 적은 초판 규율에서는 2회 모두 Advisor가 직접 수정해 재현에 실패 — Step 0 규율을 **동작 변경 기준**으로 구체화한 뒤 재현됐습니다 (Step 0 아래 상자 참고). ② 장면 2 — 한 턴에 Task 2개 동시 발사, 브리프는 모듈별 분리(상대 모듈은 [범위 경계] 제외 목록으로만 등장), 두 테스트 각각 재검증 후 커밋. ③ 장면 3 — frontmatter 고정 시 Advisor·Worker 모델이 분리 집계됐고, `CLAUDE_CODE_SUBAGENT_MODEL` 설정 시 Worker 메시지까지 전부 환경변수 값으로 바뀜(고정 모델 0건) — 원본 스크립트의 경고가 실측으로 확인됨. ④ 장면 4 — Task 0회, Advisor가 직접 Edit로 오타 수정. 참고: 이 CLI 버전의 stream-json 로그에는 Task 도구가 `Agent`라는 이름으로 찍힙니다.
