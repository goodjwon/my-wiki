...
Martin Fowler가 정리한 **이름 있는 작은 단계의 카탈로그**입니다. "외부 동작은 그대로 두고 내부 구조를 개선"하는 모든 변경을 **24가지 악취(code smell)** ↔ **66+ 리팩터링 기법** 으로 매핑해 정리한 책입니다.

> **본질**: 즉흥적 "코드 수정"과 리팩터링의 차이는 **이름·메커니즘·단계 분할**입니다. 이름 없는 변경은 위험합니다.
...
목차에는 부 구분이 없지만 흐름상 3부로 묶을 수 있습니다.
...
리팩터링의 트리거입니다. 이 책에서 가장 자주 인용되는 자산입니다.
...
→ 총 **약 66개 리팩터링 기법**입니다. 각 기법은 책의 정해진 형식(이름·예시·동기·절차·예제) 으로 카탈로그화됩니다.
...
리팩터링 카탈로그의 핵심 원리는 모든 변경에 **이름·전제·절차**를 부여하는 것입니다. 즉흥적 코드 수정과 이름 있는 리팩터링의 차이는 안전성·재현성에서 결정적입니다.

같은 원리가 위키의 다른 영역에 누적되어 있습니다:
...
→ **공통 원리**: 안전한 시스템은 **이름·전제·절차가 명시된 메커니즘**의 누적입니다. "잘 알아서 한다"는 가정은 어디서나 위험합니다.

## 세 책의 관계 — 오브젝트·Effective Java·리팩터링

같은 OO 설계 주제를 5권이 서로 다른 단위·시점으로 가리킵니다.
...
> ***리팩터링* 의 자리** : OO 원칙으로 **이미 짠 코드를 개선하는 카탈로그**입니다. 다른 4권이 처음 짤 때·매번·1줄·1사이클이라면 *리팩터링* 은 **이미 있는 코드에 적용하는 1단계 변환의 사전**입니다. 24개 악취와 66개가 넘는 기법을 담고 있습니다.
...
세 책이 같은 결론을 다른 길로 가리킵니다 → 위키에 누적되면 강력한 교차 참조 자산이 됩니다.
...
위 중 2개+ → 3장(악취)·6장(기본 리팩터링)·4장(테스트) 우선 발췌합니다.

## 한계·주의

- **2판의 예제 언어는 JavaScript입니다.** Java/Kotlin 개발자는 시그니처가 약간 다릅니다. 단, 원칙은 100% 동일합니다
- **자동화 도구 의존**: IntelliJ/Eclipse의 리팩터링 메뉴를 능숙하게 쓸수록 가성비가 폭증합니다. 수동 리팩터링은 위험합니다
- **거대 시스템 아키텍처 변경 가이드는 부족**: 책은 함수·클래스 단위입니다. 모듈·서비스 단위는 *Working Effectively with Legacy Code*(Feathers) 등으로 보완합니다
- **테스트 작성 자체는 다른 책으로**: 4장은 "왜·언제" 만 다루고, "어떻게"는 *xUnit Test Patterns*(Meszaros) 또는 *TDD*(Beck) 이 필요합니다
