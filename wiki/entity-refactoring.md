...
같은 OO 설계 주제를 세 책이 **삼각형 관계**로 다룬다.

| 측면 | [[entity-object]] *오브젝트* | [[entity-effective-java]] *Effective Java* | (이 책) *리팩터링* |
|------|----------|------|--------|
| **관점** | 책임 주도 설계 (이상적 목적지) | 90 권고 (API 설계자의 매뉴얼) | 카탈로그 (현재 코드를 거기까지 가는 길) |
| **단위** | 객체·협력·역할 | 메서드·필드·생성자 | 리팩터링 1단계 |
| **시점** | 처음부터 잘 설계 | 매번 코드 짤 때 권고 따름 | 이미 짠 코드 점진 개선 |
| **언어** | Java | Java | JavaScript (2판) — 단, 원칙은 무관 |
| **분량** | 약 700페이지 (시나리오 깊이) | 약 400페이지 (90 아이템) | 약 580페이지 (66+ 카탈로그) |
