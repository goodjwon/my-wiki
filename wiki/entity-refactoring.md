...
리팩터링 카탈로그의 핵심 원리: **모든 변경에 이름·전제·절차**입니다. 즉흥적 코드 수정과 이름 있는 리팩터링의 차이는 안전성·재현성에서 결정적입니다.
...
> ***리팩터링* 의 자리** : OO 원칙으로 **이미 짠 코드를 개선하는 카탈로그**입니다. 다른 4권이 처음 짤 때·매번·1줄·1사이클이라면 *리팩터링* 은 **이미 있는 코드에 적용하는 1단계 변환의 사전**입니다. 24 악취 + 66+ 기법.
...
- **언어 예제가 JavaScript** (2판). Java/Kotlin 개발자는 시그니처가 약간 다릅니다. 단, 원칙은 100% 동일합니다
