...
Loop 엔지니어링은 **에이전트에게 프롬프트를 입력하는 '당신 자신'을 시스템으로 대체**하는 일입니다. 사람이 매번 프롬프트를 타이핑하는 게 아니라, 에이전트를 반복적으로 구동시키는 프로그램(루프) 자체를 설계합니다.
...
→ 1·3은 [[concept-multi-agent-pattern]]의 Planner/Coder, 2는 [[concept-claude-md]]의 컨텍스트 인계, 4는 [[concept-claude-hooks]]의 back-pressure로 일부 구현되어 있습니다.
...
→ 새로 만들 게 많지 않습니다. 이미 흩어져 있던 조각이 "메커니즘 그 자체"라는 단어 하나로 묶이는 셈입니다.

## 비용 — 신중론

Addy Osmani는 토큰 비용에 **절대적으로 주의**해야 한다고 말합니다. 토큰이 넉넉한지 부족한지에 따라 사용 패턴이 크게 달라집니다.
...
- **다이어그램·표준 구현 없음**: 2026-06 시점 개념만 풀린 상태입니다. Steinberger도 저장소 링크 없이 두 문장만 던졌습니다.
- **종료 조건의 일반화 어려움**: 단순 함수는 unit test가 verify지만, 디자인·리서치 태스크는 verify를 어떻게 만들지가 열린 문제입니다.
- **모델 비용 폭증 위험**: 종량제 전환과 맞물려 비용 통제가 새 1순위 과제입니다.
