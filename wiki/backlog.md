...
### 2026-07-04
- ✅ **원본 블록 수 전수 대조 (97개 문서)** — Notion 재수집으로 블록 수·꼬리·커버리지 3단 감사. 실누락 17건을 8개 챕터에 병합(최대: ch10 §8~11 통째, ch11 11.23 §11~17, ch05 5.8 원문 전체). **원본 불신 검증 정책 신설**(CLAUDE.md 보강 정책 6): 공식 문서·소스코드 대조로 원본 오류 교정(ch01 C/C++ 표, ch06 Boot 3.x 로그 문구, ch10 JVM 옵션 등) + URL 17건 실접속 확인. ~~보류: ch03 3.9 링크 4건~~ → 링크 주석(annotation) 직접 추출로 같은 날 복원 완료 — **감사 보류 항목 0건**.
- ✅ **java-study 12챕터 §7-8 전수 점검** — 보강 약 120곳(용어 첫 등장 정의·예제 의도 리드인·hello-java/demo 재소개·댕글링 참조 정리) + **붙은 리스트 렌더 버그 89건 일괄 수정**(실증: 문단·펜스·표 직후 리스트는 한 줄로 붙음, 헤딩 직후만 정상 — 자동 스크립트) + ch00 daybyspring 잔재 demo 정합. **잔여 발견 전부 종결**: ~~ch01 [참고 1] 2.1 빈 절~~(Notion 표 유실 → 7행 비교표 복원), ~~ch11 11.90 잘림·11.23 펜스·`‣` 잔재~~ → **같은 날 Notion published API 재수집으로 전량 복구 완료**(loadPageChunk 커서 페이지네이션 + 토글 블록 직접 요청 + 표 블록 컬럼오더 추출, raw+wiki 동시). 이어서 **표 유실 전수 점검**으로 11.25 비교 표 8개 복구 + raw 마커 23건(ch07 22·ch08 1) wiki 복구본 역이식 — **저장소 전체 `<!-- table -->`·`‣`·펜스 불균형 잔존 0**.
- ✅ **§7-8 "불쑥 등장 금지(선행 소개)" 신설 + harness 8편 전수 보강** — 4규칙(블록 리드인·표기 첫 등장 정의·태스크 의도 문장·산출물 반문장 재소개)을 표준·CLAUDE.md 셀프체크에 명문화하고 8편 70여 곳 보강(용어 정의: STOP·hooks·PreToolUse·Planner/Coder/Critic·Rippable·메아리방 등). §7-7에 언어 무관 원칙(Python·C 확장 대비)도 추가.
- ✅ **harness 가이드 8편 실습 정비** (prerequisites·demo·module1~5·loop demo) — java-study의 `!!! example` 박스를 `"실습 위치·실행"` 형식으로 이식(44개), 붙은 리스트(한 줄 렌더 버그) 25건+ 분리, `> 원본: raw/...` 배너 제거, 8편 전부 도입 "왜"+"진행 흐름"·Step별 연결문으로 개연성 보강. 부수 수확: **실습이 실제로 안 되던 모순 교정** — demo guard.sh exit 1→2, M2 태스크 A 재실행 불가(revert 출발선 신설), M2→M5 revert 연쇄(`harness(M2-A)` 커밋 규약), module3 "8종 차단"→"6종+2경고" 등. 8편 lint 0건·빌드 통과.
