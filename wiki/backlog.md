...
- ✅ **harness 가이드 8편 실습 정비** — `!!! example "실습 위치·실행"` 박스 44개 이식, 붙은 리스트 25건+ 분리, `> 원본: raw/...` 배너 제거, 도입 "왜"+"진행 흐름"·Step 연결문으로 개연성 보강. 부수 수확(실습이 실제로 안 되던 모순): demo guard.sh exit 1→2, **M2 태스크 A 재실행 불가 → revert 출발선 신설 + M2→M5 revert 연쇄**(`harness(M2-A)` 커밋 규약), module3 시간·차단 종수 교정.
