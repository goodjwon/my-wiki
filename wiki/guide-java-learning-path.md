...
> [[src-java-study-2024-2025]]의 원본 교재를 **"언어 → 프레임워크 → 고급"** 세 단계로 재구성했습니다. 챕터 본문은 위에서 아래로 읽는 레퍼런스이고, 트랙(학습 코스)은 같은 내용을 미니프로젝트로 손에 익히는 동선입니다.

**학습 원칙**: 문법을 외우는 게 아니라 **왜 그렇게 동작하는지 설명**할 수 있는 상태를 목표로 합니다. 각 단계 끝의 🛠 미니프로젝트로 굳힙니다.
...
3단계가 **지식 계층**(무엇에 속하나)이라면, T1~T5 트랙은 **미니프로젝트 중심 학습 동선**(어떻게 손에 익히나)입니다. 처음 배우는 사람은 트랙을 따라가고, 필요한 주제를 레퍼런스로 찾을 땐 3단계 본문을 쓰면 됩니다.
...
- **미니프로젝트 중심**: 모든 트랙이 🛠 실습으로 끝납니다. 풀이 원문은 `raw/java-study/`에 있습니다.
- **TDD 흐름 권장**: 미니프로젝트는 빨강→초록→리팩터로. 작은 단위 검증이 막힘을 줄입니다.
