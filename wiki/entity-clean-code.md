...
같은 OO/품질 주제를 네 책이 **다른 단위·다른 시점·다른 진입**으로 다룬다.

| 측면 | [[entity-object]] *오브젝트* | [[entity-effective-java]] *Effective Java* | [[entity-refactoring]] *리팩터링* | (이 책) *Clean Code* |
|------|----|----|----|----|
| **관점** | 책임 주도 설계 (목적지) | 90 권고 (매뉴얼) | 카탈로그 (가는 길) | 미시 규칙 (가독성 + 휴리스틱) |
| **단위** | 객체·협력·역할 | 메서드·필드·생성자 | 리팩터링 1단계 | **줄·이름·함수** |
| **시점** | 처음부터 잘 설계 | 매번 코드 짤 때 | 이미 짠 코드 개선 | 매 라인·매 PR |
| **언어** | Java | Java | JavaScript (2판) | Java |
| **분량** | ~700p | ~400p | ~580p | ~580p |
| **누구에게** | 3~10년차 설계 | 1~5년차 권고 | 레거시 다루는 사람 | **신입~3년차 가독성** |

> **추천 순서 (4권 확장판)**:
> 1. *Clean Code* 2·3·4·10장 — 줄·이름·함수·클래스의 기본기 (가장 빠른 효과)
> 2. *Effective Java* 2·3·8·10장 — Java 권고 90개
