...
Robert C. Martin(Uncle Bob)이 정리한 **코드를 "깨끗하게" 만드는 미시 규칙 모음**입니다. 이 책은 이름·함수·주석·형식·오류 처리부터 동시성까지를 **줄 단위·메서드 단위의 가독성 카탈로그**로 정리합니다.

> **본질**: 깨끗한 코드는 "**다른 사람이 이해하기 쉬워야 합니다**" 라는 한 줄이 책 전체를 관통합니다. 컴파일러는 어떤 코드든 받지만, 사람은 그렇지 않습니다.
...
| 메시지 | 출처 절 | 풀이 |
|--------|---------|------|
| **나쁜 코드는 곱셈으로 비용을 키운다** | 1장 "나쁜 코드로 치르는 대가" | 한 줄의 나쁜 코드가 다음 줄을 더 나쁘게 만듭니다. 원대한 재설계는 오지 않습니다 |
| **보이스카우트 규칙** | 1장 | "체크아웃할 때보다 깨끗하게 체크인하라". 매 PR에서 약간씩 정리 |
| **우리는 저자다** | 1장 "우리는 저자다" | 코드는 후대(자기 자신 포함)가 읽는 글입니다. 작성 비율 < 읽기 비율 (1:10 추정) |
| **이름이 의도를 드러내야 한다** | 2장 | 이름 짓기에 시간을 아끼지 마라. 책임·계약·도메인을 모두 이름이 말합니다 |
| **함수는 작아야 한다 (한 가지만)** | 3장 | "한 함수는 한 가지 일만". 추상화 수준 한 단계만 내려갑니다 |
...
| **TDD의 3법칙 + F.I.R.S.T.** | 9장 | 테스트가 깨끗해야 코드가 깨끗하게 유지됩니다 |
| **단순한 설계 4 규칙 (켄트 벡)** | 12장 | (1) 모든 테스트 통과 (2) 중복 없음 (3) 의도 표현 (4) 최소 클래스·메서드 |

## 책의 구조 — 3부

목차에 부 구분은 없지만 흐름상 3부로 묶을 수 있습니다.
...
책의 마지막 장이자 카탈로그입니다. **C·E·F·G·J·N·T 7개 카테고리에 66개 항목**을 담고 있습니다.
...
Clean Code 전반을 관통하는 원리: **코드의 1차 독자는 컴파일러가 아니라 사람**입니다.

같은 메시지가 위키의 다른 영역에 누적되어 있습니다:
...
| 독자 | 효과 |
|------|------|
| **신입~3년차** | 매일 짜는 코드의 **줄 단위 가독성** 이 즉시 좋아집니다. 가장 빠른 효과를 보는 책 |
| **코드 리뷰어** | 17장 휴리스틱 코드(G19, N1, T5...) 로 리뷰 어휘 표준화 |
| **신입 교육** | 1·2·3장이 통째로 교육 자료입니다. 보이스카우트 규칙은 팀 문화입니다 |
| **레거시 만지는 사람** | 14·15·16장의 점진 개선 사례 + 17장 휴리스틱 |
| **테스트 입문자** | 9장 F.I.R.S.T.·TDD 3법칙이 가장 짧은 입문입니다 |
...
위 항목 중 2개 이상에 해당하면 1·2·3·10·17장을 먼저 발췌해 읽습니다.

## 한계·주의

- **2008년 출판** — 일부 코드 스타일(예: SerialDate)·테스트 도구(JUnit 3·4)는 현대(JUnit 5·record·sealed class)와 차이가 있습니다. 단, 원칙은 100% 유효합니다
- **"무조건적 규칙" 인용 위험** — 책의 일부 권고("주석 거의 다 나쁘다", "함수 4줄 이내")는 맥락을 떼면 도그마가 됩니다. 책 본문이 항상 "단, ..." 단서를 다는 점을 유의해야 합니다
- **함수형·반응형 부재** — Stream·CompletableFuture·Reactor 같은 모던 함수형 사고는 다른 책으로 보완해야 합니다 ([[entity-effective-java]] 7장)
- **아키텍처는 거의 다루지 않습니다** — 후속작 *Clean Architecture* 가 다루는 영역입니다
- **17장 휴리스틱은 외울 게 아닙니다** — 필요할 때 펼쳐서 검색하는 사전으로 씁니다

## 원본 출처

- `raw/clean-code/toc.md` — 사용자가 입력한 목차 (2026-06-20)
- `raw/clean-code/클린 코드 실전 강의 교재 1~17장.md` — 17편 강의 교재 (사용자 입력 1·2장 + 본 세션 작성 3~17장, 약 5,500줄). 책 본문 직접 인용이 아닌 강의용 재구성입니다.
