...
Kent Beck 이 정리한 **TDD 의 원전**입니다. 화폐 예제 하나로 빨강·초록·리팩터 사이클을 장마다 직접 보여주고, xUnit 자체를 TDD 로 만들고, 마지막에 패턴 카탈로그로 마무리합니다.

> **본질**: "**빨강 → 초록 → 리팩터**" 의 짧은 사이클이 좋은 설계를 **창발** 시킵니다.
...
→ 17장 분량의 작은 단계가 정교한 도메인 모델 (Money·Bank·Expression·Sum) 을 만듭니다.
...
TDD 개발자는 코드를 짜기 전에 먼저 실패하는 테스트를 작성합니다. 테스트 막대가 빨갛게 물들면 시스템이 "아직 그 기능은 없다"고 스스로 거부하고, 개발자는 그 거부를 지우는 최소한의 코드만 씁니다.

이 빨강이 바로 사이클 안에 붙박이로 들어가 있는 **거부 신호**입니다. 위키의 다른 영역에도 같은 패턴이 누적되어 있습니다.
...
위 중 2개+ → 1·2·3·4·5장 (1부 시작) + 25·28장 (패턴) 을 우선합니다.

## 한계·주의

- **2002년 출판** — Java 5 이전. 일부 코드 스타일은 현대 (record·sealed) 와 차이가 있습니다
- **모든 코드에 TDD 100% 적용은 어려움** — UI·통합·성능은 다른 도구가 필요합니다
- **Fowler 의 마치는 글** 이 균형 잡힌 시각을 보여줍니다 — Test-First vs Test-After, Mock 의존도
- **숙련도가 결정적** — 책 읽기만으로는 부족하며, 직접 코딩이 필수입니다
