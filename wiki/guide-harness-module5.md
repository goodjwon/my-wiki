...
주간 리뷰는 세 단계로 진행한다. 실패 흔적을 **수집**(3-1)하고, 그 결과를 리뷰 파일에 **분류·정리**(3-2)한 뒤, 정리된 패턴을 **규칙으로 전환해 커밋**(3-3)한다. 앞 단계의 산출물이 다음 단계의 입력이 되는 피드백 루프다.
...
위 프롬프트를 실행해 나온 실패 패턴과 분류 결과를 아래 템플릿에 옮겨 적는다. 즉 3-1의 출력이 이 리뷰 파일의 입력이 된다.
...
위 리뷰 파일에 정리한 분류 A·B 패턴을 이제 실제 규칙으로 옮긴다. CLAUDE.md 섹션 7과 guard.sh에 반영한 뒤, 리뷰 파일까지 함께 커밋한다.
