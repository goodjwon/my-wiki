...
AI 에이전트가 **잘못된 일을 구조적으로 못 하게 만드는 시스템**을 설계하는 일입니다. 프롬프트(부탁) 대신 환경(구조)으로 에이전트의 행동 경계를 제어합니다.
...
OpenAI 5개월 내부 실험: 100만 줄의 코드를 생성했지만 인간이 작성한 코드는 0줄이었습니다. **병목은 모델이 아니라 환경이었습니다.**

2026-06 Peter Steinberger의 X 게시물("이제 에이전트에게 프롬프트를 주는 루프를 설계해야 한다", 650만 조회) 이후 **하네스의 다음 단계로 "루프"가 호명**되기 시작했습니다. 하네스가 환경이라면, 루프는 그 환경 안에서 사람이 매번 입력하던 자리까지 자동화하는 단계입니다. → [[concept-loop-engineering]] 참조.

## 왜 프롬프트로는 부족한가

> **교통 규칙 vs 물리적 구조물**
> "이 길로 가지 마세요"라는 표지판(프롬프트)은 무시될 수 있습니다.
> 중앙 분리대(시스템/구조)는 물리적으로 막습니다.
> 프롬프트는 표지판이고, 하네스는 중앙 분리대입니다.

구체적 한계:
1. **희석 효과**: 대화 맥락이 길어질수록 초반 지시가 잊힙니다.
2. **부정 명령의 역설**: `NEVER do X` 같은 부정 명령은 모델이 내부적으로 X를 먼저 떠올린 후 억제하는 구조 → 실패율이 높습니다.
3. **검증 불가능**: 프롬프트는 "지켰는지" 외부에서 확인할 수 없지만, hook은 실행 여부로 확인 가능합니다.
