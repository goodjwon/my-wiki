...
Joshua Bloch(JDK 컬렉션·`java.lang` 핵심 설계자)가 자바 API를 만들며 쌓은 **90개 권고**를 "아이템" 단위로 모은 책입니다. 한 권을 통째로 읽기보다 **언제든 펼쳐 보는 참조서**로 쓰는 편이 일반적입니다.

> **비유 — "정비사의 매뉴얼"**
>
> 오래된 정비소의 작업대에는 두꺼운 정비 매뉴얼이 한 권 놓여 있습니다. 정비사는 이 매뉴얼을 첫 장부터 끝까지 읽지 않습니다. 시동이 걸리지 않는 차가 들어오면 해당 증상이 실린 페이지를 바로 펼치고, 앞선 정비사들이 같은 고장을 수없이 고치며 검증해 둔 절차를 그대로 따릅니다.
>
> *Effective Java*는 자바 개발자에게 이 정비 매뉴얼과 같은 책입니다. 90개 아이템 하나하나가 증상별 페이지에 해당하고, equals 재정의·생성자 인자 폭증·직렬화 같은 상황을 만나면 해당 아이템을 펼쳐 저자가 JDK API를 만들며 검증한 처방을 그대로 가져옵니다. 그래서 이 책은 "자바로 짤 수 있는가"가 아니라 "자바로 잘 짜는가"를 다루며, 언어 문법보다 **API 설계·성능·안전성**에 쌓인 지혜를 전합니다.
...
Effective Java 전반을 관통하는 또 하나의 원칙: **한 번 공개된 결정은 영원히 가져갑니다**. 그래서 신중해야 합니다.
...
→ **공통 원리**: 공개 인터페이스(API·시그니처·스키마·직렬 형태)는 결정 시점에 신중을 다해야 합니다. 후일 변경 비용이 사용자 수·데이터 양에 비례해 폭증합니다.

## *오브젝트*와의 관계

같은 OO 설계 주제를 **두 책이 보완 관계**로 다룹니다.
...
| 독자 | 효과 |
|------|------|
| **1~3년차 자바 개발자** | "왜 이렇게 짜야 하나" 가 비로소 연결됩니다. 코드 리뷰에서 자주 지적받는 항목들의 출처 |
...
위 항목 중 2개 이상에 해당하면, 해당 Item부터 발췌해서 읽는 편이 가성비가 가장 좋습니다.
