...
**왜 데모부터 보나**: 하네스는 규칙·hook·프로세스를 쌓아 올리는 장치라, 설명만 들으면 추상적으로 느껴집니다. 여기서 hook은 에이전트가 명령을 실행하기 직전에 자동으로 끼어들어 그 명령을 검사하는 스크립트를 말합니다 — Step 3에서 직접 하나 만들어 봅니다. 사고가 실제로 나는 장면과 같은 사고가 자동 차단되는 장면을 5분 안에 연달아 겪어 두면, 이후 5모듈에서 각 장치를 왜 만드는지가 체감으로 남습니다.
...
데모 디렉터리(`~/harness-demo`)에서 `claude`를 입력해 에이전트와의 대화 세션을 엽니다.
...
아래가 사고 유발용 프롬프트입니다 — 시크릿 커밋을 그럴듯한 이유로 포장해, 에이전트가 스스로 걸러 내는지 봅니다.
...
Claude가 정말 커밋했는지 git 이력에서 직접 확인합니다.
...
방금 시크릿이 git 히스토리에 박히는 사고를 봤으니, 이번에는 같은 요청을 시스템이 막게 만듭니다. 설치할 장치는 서두에서 소개한 hook 한 개 — `.env`를 건드리는 git 명령을 발견하면 차단하는 셸 스크립트 `guard.sh`를 만들어 Claude Code에 hook으로 등록합니다. 5모듈을 다 보고 만들 정식 hook이 아니라 **데모용 미니 버전**입니다.
...
이 데모는 하네스의 **딱 1개 측면**(시크릿 파일 차단)만 보여줬습니다. 실제 5모듈에서는 아래 장치들이 차례로 추가됩니다 — 표 속 이름이 지금은 낯설어도 괜찮습니다. 각 모듈에서 처음부터 하나씩 만듭니다.
