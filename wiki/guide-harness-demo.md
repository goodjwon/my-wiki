...
먼저 데모용 빈 디렉터리를 만들고, 시크릿 흉내용 `.env`와 일반 코드 파일을 넣어 둡니다.
...
`harness-demo`는 데모 전용이므로 삭제합니다. 5모듈 실습은 [[guide-harness-00-prerequisites]] 의 `~/harness-playground`에서 따로 진행합니다.
