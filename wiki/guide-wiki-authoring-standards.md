...
2. **표기·규약은 첫 등장에서 정의** — 프롬프트 머리 대괄호 표시(`[베이스라인 측정 중]`), 커밋명 규약(`harness(M2-D)`), 판정 용어(APPROVE / CONDITIONAL REJECT) 같은 문서 내 관례는 처음 쓰는 자리에서 한 문장으로 정의합니다. 이후 줄임형을 쓰면 "같은 표시의 줄임"임을 밝힙니다.
