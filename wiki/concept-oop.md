...
객체지향 프로그래밍은 상태(state)와 행위(behavior)를 객체 단위로 묶고, **책임(responsibility)을 객체들 사이에 분배**하는 프로그래밍 패러다임입니다. Java·C#·Kotlin·Python·Ruby 등의 언어가 이 철학을 핵심 설계 기반으로 삼습니다.

극장에서 이벤트 공연을 연다고 해 봅시다. 극장 주인이 관람객의 가방을 직접 뒤져 초대장을 꺼내고, 매표소 서랍을 직접 열어 돈을 센다면 공연은 어떻게든 굴러가겠지만, 무례하고 부자연스럽습니다. 관람객은 스스로 표를 사고 판매원은 스스로 표를 팔 때, 비로소 각자가 자기 일을 자기가 하는 자연스러운 극장이 됩니다.

객체지향의 본질도 같은 그림입니다. 하나의 함수가 모든 데이터를 꺼내 와 처리하는 대신, 각 **객체**가 자기 **데이터**를 끌어안고 그 데이터에 대한 **책임**을 스스로 집니다. 호출자는 할 일을 부탁하는 메시지만 보내고, 그 일을 어떻게 해내는지는 맡은 객체가 알아서 결정합니다. "함수가 데이터를 처리한다"에서 "객체가 자기 데이터에 책임을 진다"로의 관점 전환이 이 패러다임의 출발점입니다.

> 한국어 표준 해설서로는 조영호 *오브젝트* (위키북스, 2019) → [[entity-object]] 를 참고합니다. 위 극장 장면이 바로 그 책 1장의 예제이고([[lecture-object-ch1]]), 책의 핵심 메시지 "메시지가 객체를 결정한다 / 행동이 상태를 결정한다"가 위의 관점 전환과 같은 이야기입니다.
