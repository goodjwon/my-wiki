...
준비한 템플릿으로 이슈를 만들 때, 제목·라벨·마일스톤은 아래처럼 함께 지정합니다.
...
아래 두 항목은 회원관리 기능의 하위 요구사항 페이지 제목입니다. 상세 명세는 원고 데이터베이스의 하위 페이지로만 유지하고, 이 부록에는 제목만 남깁니다.
...
실험용 데이터베이스 역시 원고 데이터베이스의 하위 데이터베이스로만 유지하며, 이 부록에는 옮기지 않습니다.
...
구현에 앞서 아래 스크립트로 `mini_blog` 데이터베이스와 테이블을 만듭니다. 11.10에서 잡은 DB 모델링 초안(Users·Categories·Posts·Comments)에 세션 테이블(`user_sessions`)과 권한·신고 컬럼을 더한 형태입니다.
...
실습 프로세스의 "모델(Model) 구현" 단계입니다. 아래 클래스들은 2번 트리의 `src/main/java/com/myblog/` 아래에 만들며, 각 소제목의 괄호 안이 파일 위치입니다.
...
실습 프로세스의 "뷰(View) 및 로직 처리 구현" 단계입니다. 파일은 `src/main/webapp/` 아래 경로에 만들며, 로그인 폼 → 로그인 처리 → 로그아웃 순으로 인증 흐름부터 봅니다.
...
앞의 `SessionUtil`에서 TODO로 남겨 둔 Remember Me 토큰의 DB 저장·조회를 맡는 DAO입니다.
...
war 배포용 Gradle 빌드 설정입니다. 프로젝트 루트(`my-blog/`)에 둡니다.
...
웹 애플리케이션 배포 서술자입니다. `src/main/webapp/WEB-INF/web.xml`에 두고, 문자 인코딩 필터·세션 쿠키·에러 페이지를 설정합니다.
...
절 제목의 (숙제) 표시는 완성 답안을 싣는 대신 독자가 직접 채워 보는 항목이라는 뜻입니다. 아래는 위 시나리오를 "누가 무엇을 어떻게 할 수 있는가" 기준으로 행위자별 체크리스트로 정제한 예시입니다.
...
아래는 기능 항목 옆에 조건과 비고를 나란히 적어 둔 명세 초안 예시입니다. 11.10의 최소 형식(입력·성공 결과·실패 조건)으로 다듬으며 채워 갑니다.
...
(숙제) 11.10의 DB 모델링 초안(Users·Categories·Posts·Comments)을 출발점으로 직접 작성합니다.

### 객체 모델링

(숙제) 11.10의 "객체 모델링으로 옮길 때 볼 포인트"를 참고해 핵심 클래스와 책임을 직접 정의합니다.

### 주요 SQL 쿼리 (숙제)

11.10의 "주요 SQL 또는 API로 확장하는 방법"을 참고해 기능별 대표 쿼리를 직접 적습니다.
...
#### 3단계: Spring 프레임워크 적용
- 2단계의 서블릿 MVC 구조를 Spring 프레임워크(IoC 컨테이너, Spring MVC)로 리팩토링

### 요구사항 비교 분석 (참고 답안)

위 "요구사항 시나리오 정제 (숙제)"를 실제로 수행한 비교 분석 예시입니다. 원본 시나리오 → 정제된 요구사항 → 제공된 체크리스트를 대조해 누락과 불일치를 찾는 과정을 보여 줍니다. 숙제를 먼저 스스로 해 본 뒤 참고 답안으로 읽는 것을 권장합니다.
