...
### 유지본 연결
- JVM 관점으로 이어서 읽기: JVM 기초 가이드 1: 개념과 구성 원리 (→ 10.0)
- 메모리 문제로 이어서 읽기: JVM 기초 가이드 2: 메모리 관리 (→ 10.1)
- 튜닝 관점으로 이어서 읽기: JVM 기초 가이드 3: 튜닝과 실전 활용 (→ 10.2)
...
### 유지본 연결
- 소켓 프로그래밍: 네트워크 프로그래밍 기초 (→ 5.5)
- 서버와 실행 환경: Tomcat 실행과 설정 (→ 8.0)
- 인증 흐름: Spring Security 인증 흐름 (→ 8.1)
- 토큰 기반 인증: 토큰 기반 인증 (→ 8.3)
...
### 유지본 문서 지도
- 컴퓨터 구조와 메모리: 11.2 컴퓨팅 구조 (→ 11.2), 10.0 JVM 기초 가이드 1: 개념과 구성 원리 (→ 10.0), 10.1 JVM 기초 가이드 2: 메모리 관리 (→ 10.1)
- 네트워크: 11.3 네트워크 구조 (→ 11.3), 5.5 네트워크 프로그래밍 기초 (→ 5.5)
- 데이터베이스: 7.0 데이터베이스 설계 (→ 7.0), 5.6 JDBC 기초 (→ 5.6), 7.3 쿼리 최적화 (→ 7.3)
...
### 개요
이 문서는 책 본문을 읽다가 `DBMS`, `RDBMS`, `NoSQL` 같은 용어가 헷갈릴 때 빠르게 다시 보기 위한 참고 문서입니다. 본문 중심 설명은 데이터베이스 설계 (→ 7.0)에 두고, 이 문서는 개념 구분만 간결하게 정리합니다.
...
- Consistency: 제약조건을 깨지 않는 상태를 유지해야 합니다.
- Isolation: 동시에 실행되는 작업끼리 서로를 함부로 오염시키지 않아야 합니다.
- Durability: 커밋된 결과는 장애가 나도 보존되어야 합니다.
Java 서비스에서 `@Transactional`을 쓰는 이유도 결국 이 성질을 코드 레벨에서 다루기 위해서입니다.

### COMMIT과 ROLLBACK
- `COMMIT`: 트랜잭션 결과를 확정합니다.
- `ROLLBACK`: 트랜잭션 중간 변경을 취소합니다.
...
- 예외: 예외와 사용자 정의 예외 (→ 5.2)
- JVM: JVM 기초 가이드 1: 개념과 구성 원리 (→ 10.0)
...
### 권장 구현 순서
1. 오프라인 단일 사용자 버전부터 만듭니다.
1. `player_data.txt` 또는 JSON 파일로 상태 저장을 붙입니다.
1. 전투 로그를 파일로 남깁니다.
1. 서버 없이도 동작하는 최소 게임 루프를 만듭니다.
1. 그 뒤에 소켓 또는 HTTP 기반 동기화를 붙입니다.
...
패키지 선언 없는 단일 파일 실습입니다. 아무 디렉터리에나 아래 파일 하나를 만들어 컴파일·실행합니다.

**파일**: MemoryMonitor.java
...
(수치는 JVM·힙 설정에 따라 다르며, 부하 테스트 후 `System.gc()` 요청으로 사용 메모리가 줄어드는 흐름만 확인하면 됩니다.)
...
**문제:**
```java
public class CachedUserSession {   // 11.23 블로그의 DTO UserSession과 별개인 캐시 예시
