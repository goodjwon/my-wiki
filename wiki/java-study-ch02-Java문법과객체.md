...
**트랙**: [[guide-java-track1-basics]] · **다음 장**: [[java-study-ch03-컬렉션과함수형]]
...
**🎯 목표**: 변수·기본형/참조형·형 변환·연산자를 구분해 쓰고, `==`와 `equals`의 차이를 코드로 설명한다.
...
조건문은 어떤 상황에서 어떤 코드를 실행할지 결정합니다. 핵심은 문법보다 **분기 기준이 명확한가**입니다.
...
```
이때 중요한 것은 위에서부터 읽히므로, **더 구체적이거나 더 우선순위가 높은 조건을 먼저 배치해야 한다**는 점입니다.

### 2. `switch`는 값 중심 분기에 적합하다
`switch`는 하나의 값을 기준으로 여러 경우를 나누는 데 적합합니다.
```java
switch (day) {
...
```
실무에서는 `for`와 `while`이 훨씬 자주 보이고, `do-while`은 제한적으로 사용됩니다.

### 4. 반복 제어 키워드도 함께 이해해야 한다

#### `break`
가장 가까운 반복문이나 `switch`를 즉시 종료합니다.

#### `continue`
현재 반복만 건너뛰고 다음 반복으로 넘어갑니다.
이 두 키워드는 편리하지만, 많이 쓰면 흐름이 눈에 잘 들어오지 않을 수 있으므로 조건 구조 자체를 단순하게 만드는 편이 더 좋은 경우가 많습니다.

### 5. 초중급 단계에서 자주 하는 실수
- `if` 조건을 중복해서 써서 분기 구조가 불필요하게 길어지는 것
- `switch`와 `if-else`를 상황에 맞게 고르지 않고 습관대로만 쓰는 것
...
### 정리
조건문은 분기 기준을 코드로 표현하는 도구이고, 반복문은 같은 작업을 안전하게 반복하는 도구입니다. 둘 다 문법 자체보다 **조건과 종료 시점을 명확하게 드러내는 것**이 핵심입니다.
...
클래스는 설계도이고, 객체는 그 설계도로부터 만들어진 실행 단위입니다.
```java
...
```
위 코드에서 `Account`는 클래스이고, `new Account("123-456", 10000)`으로 만들어진 값이 객체입니다.

### 2. 객체지향의 출발점은 데이터가 아니라 책임이다
초보 단계에서는 객체를 필드 묶음처럼 보기 쉽습니다. 하지만 객체지향에서는 `무엇을 가지고 있는가`보다 `무엇을 책임지는가`를 먼저 보는 편이 더 정확합니다.
...
```
이 구조가 중요한 이유는, 외부 코드가 `balance`를 직접 수정하지 못하고 객체의 규칙을 통과해야 하기 때문입니다.
...
다형성의 핵심은 부모 타입으로 다루더라도 실제 구현에 따라 동작이 달라지는 것입니다.
```java
...
추상화는 복잡한 세부 구현을 가리고, 사용자가 알아야 할 최소한의 계약만 드러내는 방식입니다. 인터페이스는 특히 협력 지점을 분리할 때 강력합니다.
...
이 기준이 잡히면 객체를 더 신뢰할 수 있고, 테스트도 쉬워집니다.
...
### 실무 연결 포인트
Spring에서도 객체지향 원칙은 그대로 이어집니다. DI, 전략 패턴, 도메인 모델링, 테스트 가능한 구조는 모두 `구현이 아니라 역할에 의존한다`는 객체지향 감각 위에서 잘 작동합니다.
...
- `try-catch-finally`의 흐름을 이해하는 문제
...
### 함께 보면 좋은 추가 문제
- [게임 관련 시나리오](https://www.notion.so/1381e21f38368053be73f3189ba5ef18)는 객체 모델링, 상속, 추상 클래스, 제네릭을 한 번에 묶어 연습하는 확장 문제 모음입니다.
...
```javascript
전사이(가) 공격합니다!
전사이(가) 강력한 공격을 합니다! 남은 스태미나: 40
마법사이(가) 공격합니다!
마법사이(가) 마법을 시전합니다! 남은 마나: 80
궁수이(가) 공격합니다!
궁수이(가) 화살을 발사합니다! 남은 화살: 29
```
...
**문제**: 캐릭터의 경험치와 레벨을 관리하는 `Player` 클래스를 작성하세요. `experience`와 `level` 필드는 private으로 설정하고, 공개 메서드 `gainExperience(int amount)`를 통해서만 경험치를 증가시킬 수 있도록 하세요.
```javascript
Jwon이(가) 경험치 1000점을 획득했습니다.
Jwon이(가) 레벨 2로 상승했습니다!
Jwon이(가) 레벨 3로 상승했습니다!
Jwon이(가) 레벨 4로 상승했습니다!
...
**문제**: `Movable` 인터페이스를 정의하고 `move(int x, int y)` 메서드를 선언하세요. 이 인터페이스를 구현하는 `Player`와 `NPC` 클래스를 작성하여 캐릭터들이 게임 맵에서 이동할 수 있도록 하세요.
```javascript
용사이(가) 위치 (10, 20)로 이동했습니다.
NPC 상인이(가) 위치 (15, 25)로 이동했습니다.
용사이(가) 위치 (30, 40)로 이동했습니다.
NPC 상인이(가) 위치 (35, 45)로 이동했습니다.
```
...
#### 문제 2: **짝수만 출력하기**
**문제 설명**: 1부터 50까지의 숫자 중 짝수만 출력하는 프로그램 작성.
**출력 예시**:
```plain text
2
4
...
#### 문제 3: **숫자의 합 계산하기**
**문제 설명**: 1부터 10까지의 숫자를 더한 합계를 출력하는 프로그램 작성.
**출력 예시**:
```plain text
1부터 10까지의 합: 55
...
#### 문제 4: **구구단 출력**
**문제 설명**: 사용자가 입력한 숫자의 구구단을 출력하는 프로그램 작성.
**출력 예시**:
```plain text
사용자가 입력한 숫자가 5인 경우:
5 * 1 = 5
...
#### 문제 5: **홀수의 합 구하기**
**문제 설명**: 1부터 100까지의 숫자 중 홀수의 합을 구하는 프로그램 작성.
**출력 예시**:
```plain text
...
#### 문제 6: **팩토리얼 계산**
**문제 설명**: 사용자가 입력한 숫자의 팩토리얼 값을 구하는 프로그램 작성.
**출력 예시**:
```plain text
사용자가 입력한 숫자가 5인 경우:
5의 팩토리얼 값: 120
...
#### 문제 7: **배열 요소 합 계산**
**문제 설명**: 정수 배열이 주어졌을 때, 배열의 모든 요소의 합을 구하는 프로그램 작성.
**출력 예시**:
```plain text
...
#### 문제 8: **숫자 역순 출력**
**문제 설명**: 100부터 1까지 역순으로 숫자를 출력하는 프로그램 작성.
**출력 예시**:
```plain text
100
99
...
#### 문제 9: **최대값 찾기**
**문제 설명**: 주어진 정수 배열에서 가장 큰 숫자를 찾는 프로그램 작성.
**출력 예시**:
```plain text
...
#### 문제 10: **구간 합 구하기**
**문제 설명**: 사용자가 입력한 두 숫자 사이의 모든 숫자의 합을 구하는 프로그램 작성.
**출력 예시**:
```plain text
