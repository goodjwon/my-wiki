...
**Q. int enum 패턴이 enum보다 위험한 결정적 이유는?**

**A.** **타입 안전성이 없습니다.** 컴파일러가 "사과 상수 자리에 오렌지 상수"를 막지 못해, 의미상 잘못된 값이 그대로 통과합니다. enum은 이를 컴파일 시점에 차단합니다.
...
**Q. enum을 DB에 ordinal로 저장하면 어떤 사고가 나는가?**

**A.** 나중에 enum 상수의 **순서를 바꾸거나 중간에 추가**하면, DB에 저장된 숫자가 다른 상수를 가리키게 되어 **데이터 의미가 통째로 어긋납니다.**
...
**Q. EnumSet이 비트 필드의 장점(성능)도 가져가는 이유는?**

**A.** 내부적으로 **비트 벡터로 구현**되어 있어, 가독성·타입 안전을 얻으면서도 비트 연산 수준의 성능을 유지합니다.
...
**Q. EnumMap이 "ordinal 인덱싱 배열"의 성능을 거의 그대로 가져가는 이유는?**

**A.** 내부적으로 **배열로 구현**되어 있어, 배열 인덱싱과 비슷한 성능을 내면서도 타입 안전과 가독성을 제공합니다.
...
**Q. enum을 직접 상속(extends)할 수 없는 이유는?**

**A.** 모든 enum은 암묵적으로 `java.lang.Enum`을 상속하고 있어, 추가 상속이 불가능합니다. 그래서 확장은 **공통 인터페이스 구현**으로 풉니다.
...
**Q. `@Retention(SOURCE)` 애너테이션을 리플렉션으로 읽을 수 없는 이유는?**

**A.** `SOURCE`는 컴파일 단계까지만 존재하고 **클래스 파일/런타임에는 남지 않기** 때문입니다. 런타임 리플렉션으로 읽으려면 `RUNTIME`이어야 합니다.
...
**Q. @Override가 잡아주는 대표적 실수는?**

**A.** 재정의 의도였으나 **시그니처가 달라(예: Object 대신 구체 타입) 다중정의**가 되어버리는 실수를, 컴파일 에러로 즉시 발견하게 해줍니다.
...
**Q. 마커 인터페이스가 마커 애너테이션보다 나은 결정적 한 가지는?**

**A.** **컴파일 타임 타입 검사**가 가능합니다. 그 타입을 요구하는 매개변수/필드 자리에서 컴파일러가 강제할 수 있습니다.
...
**Q1. enum과 애너테이션을 한 문장으로 대비하면?**

**A.** enum은 "**값을 정해진 집합으로 제한**"하는 도구, 애너테이션은 "**코드에 메타데이터를 붙여 도구가 읽게**"하는 도구다.

**Q2. `@Transactional`이 6장의 어떤 아이템을 구현한 것인가?**

**A.** 아이템 39, **애너테이션**(메타데이터를 붙이고 Spring이 리플렉션/프록시로 읽어 처리).

**Q3. 계약상태를 enum으로 만들 때 얻는 가장 큰 도메인적 이점은?**

**A.** 잘못된 값 차단에 더해, **상태 전이 규칙 같은 불변식을 enum 안에 캡슐화**해 도메인이 스스로 무결성을 지키게 할 수 있다.
