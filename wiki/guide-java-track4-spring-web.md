...
| 순서 | 문서 | 한 줄 |
|------|------|------|
| 5.0 | Spring 핵심: IoC·DI·Bean·MVC | 객체 조립을 컨테이너에 위임 (→ [[concept-spring-core]]) |
| 5.1 / 5.2 | 실습 환경·Maven 구성 | 빌드·의존성 관리 |
| 5.3 | 프로파일 설정 | dev/prod 환경 분리 |

### 2단계 — 데이터 접근과 SQL (ch06)
| 순서 | 문서 | 한 줄 |
|------|------|------|
| 6.0~6.3 | DB 설계·SQL·쿼리 최적화 | 정규화·인덱스·실행계획 |
| 6.4~6.10 | Querydsl 도입~페이징 | 타입 안전 동적 쿼리 (→ [[entity-querydsl]]) |
| 6.12 | SQL 연습 문제 | 🛠 손풀기 |

### 3단계 — 서버와 인증 (ch07)
| 순서 | 문서 | 한 줄 |
|------|------|------|
| 7.0 | Tomcat 실행과 설정 | 서블릿 컨테이너 |
| 7.1~7.3 | Spring Security·토큰 인증 | 인증 흐름, JWT |

### 4단계 — 테스트와 품질 (ch08)
| 순서 | 문서 | 한 줄 |
|------|------|------|
| 8.0~8.2 | 테스트 전략·계산기 테스트 | 단위→통합 피라미드 |
| 8.3 | curl 수동 검증 | API 손으로 찔러보기 |
