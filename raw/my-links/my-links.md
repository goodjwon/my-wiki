...
## 1. [Design patterns in TypeScript and Node.js - LogRocket Blog](https://blog.logrocket.com/design-patterns-in-typescript-and-node-js/)
> 생성: 2021-01-28
...
## 2. [Python VS JavaScript – What are the Key Differences Between The Two Popular Programming Languages?](https://www.freecodecamp.org/news/python-vs-javascript-what-are-the-key-differences-between-the-two-popular-programming-languages/)
> 생성: 2021-01-29
...
## 3. [How to Create a CSS Grid Layout for Divi Modules](https://www.elegantthemes.com/blog/divi-resources/how-to-create-a-css-grid-layout-for-divi-modules)
> 생성: 2021-04-02
...
## 4. [개발자](http://channy.creation.net/blog/1600)
> 생성: 2022-06-30
...
## 5. [[Modern Java] 람다 표현식(Lambda Expression)](https://dev-kani.tistory.com/38)
> 생성: 2022-07-24
...
## 6. [if statement - Use Java lambda instead of 'if else' - Stack Overflow](https://stackoverflow.com/questions/49857232/use-java-lambda-instead-of-if-else)
> 생성: 2022-07-25

Java에서 if-else 대신 람다를 사용하는 방법에 대한 Stack Overflow 질문. Optional, Map, 함수형 인터페이스를 활용하여 조건 분기를 깔끔하게 처리하는 패턴을 다룬다.

---

## 7. [Java 8 함수형 인터페이스 (Functional Interface)](https://bcp0109.tistory.com/313)
> 생성: 2022-07-25
...
## 8. [Java 8 람다 표현식 자세히 살펴보기 | Devlog in the SKY](https://skyoo2003.github.io/post/2016/11/09/java8-lambda-expression/)
> 생성: 2022-07-25
...
## 9. [Mapping JPA Entities into DTOs in Spring Boot Using MapStruct](https://auth0.com/blog/how-to-automatically-map-jpa-entities-into-dtos-in-spring-boot-using-mapstruct/)
> 생성: 2022-07-28
...
## 10. [Finding Max and Min from List using Streams - HowToDoInJava](https://howtodoinjava.com/java8/stream-max-min-examples/)
> 생성: 2022-10-24

Java Stream API의 `max()`와 `min()` 메서드를 `Comparator.comparing()`과 함께 사용하여 숫자, 문자열, 날짜 및 객체에서 최댓값과 최솟값을 효율적으로 찾는 방법을 다룬다.

```java
Integer maxNumber = Stream.of(1, 2, 3, 4, 5)
...
## 11. [데일리 스크럼 : '데일리 스크럼'을 더 잘하기 위한 생각 - 컬리 기술 블로그](https://helloworld.kurly.com/blog/daily-scrum-thinking/)
> 생성: 2023-08-09
...
## 12. [소프트웨어 엔지니어링 팀은 기술 부채를 어떻게 추적합니까? | AppMaster](https://appmaster.io/ko/blog/sopeuteuweeo-enjinieoring-timeun-gisul-bucaereul-eoddeohge-cujeoghanayo)
> 생성: 2023-09-17
...
## 13. [기술 부채: 예시 및 유형 | AppMaster](https://appmaster.io/ko/blog/gisul-bucae-yesi-mic-yuhyeong)
> 생성: 2023-09-24
...
## 15. [The Spring Modulith: Monolithic but Manageable | by Ritesh Shergill | Medium](https://medium.com/@riteshshergill/the-spring-modulith-monolithic-but-manageable-ca1532a1e585)
> 생성: 2023-10-03

Spring Modulith는 도메인 기반의 모듈 구조로 모놀리식 Spring Boot 앱을 체계적으로 관리하는 프레임워크.

- 코드를 명확한 패키지와 책임으로 조직
- 모듈 간 구조적 무결성을 테스트로 검증
- 팀 간 모듈 단위 개발 분배 가능
- 마이크로서비스의 복잡성 없이 모놀리식의 혼란을 방지하는 실용적 중간 지점

---

## 16. [역량만 있어선 안 된다… IT 프로젝트가 여전히 실패하는 8가지 이유](https://www.ciokorea.com/news/311246)
> 생성: 2023-10-25
...
## 17. [개발자가 "No"라고 말할 때 물어봐야 할 4가지 질문 | GeekNews](https://news.hada.io/topic?id=13333)
> 생성: 2024-02-13

PM이 개발자의 거절에 대응하는 4가지 질문:

1. **대체 기술적 해결책이 있는가?** — 더 단순한 방법 탐색 유도
2. **제약 조건 내에서 해결책을 제시할 수 있는가?** — "당신의 해결책은?"
3. **단계적 접근이 가능한가?** — Phase 1, 2, 3으로 나누어 진행
4. **제거할 수 있는 장애물은 무엇인가?** — 불필요한 업무 제거로 시간 확보

핵심: 밀어붙이기가 아니라, 함께 해결책을 찾는 협력 자세.

---

## 18. [OROR Forge: Figma to Code 도구 제작기 (1) 디자인을 코드로 만들어보자! – tech.kakao.com](https://tech.kakao.com/2024/01/09/ororforge-1/)
> 생성: 2024-04-12
...
## 19. [OROR Forge: Figma to Code 도구 제작기 (2) 실전용으로 만들기 – tech.kakao.com](https://tech.kakao.com/2024/01/09/ororforge-2/)
> 생성: 2024-04-12

카카오의 OROR Forge — Figma 디자인을 실전용 코드로 변환하는 도구.

- 인라인 스타일 → TailwindCSS → React Component 순서로 변환
- Props, Models, Attributes, Replacements로 비즈니스 로직 유지
- 핵심 인사이트: "모든 디자인을 전부 좋은 코드로 변환할 수 없다" — 잘 구조화된 디자인 시스템이 전제조건

---

## 20. [내 장바구니](https://www.decathlon.co.kr/cart)
> 생성: 2024-10-18
...
## 21. [3개월간 개발한 마이크로서비스에 498개의 테스트 코드를 작성한 시니어 개발자가 추천해준 <The Practical Test Pyramid>을 번역했다. 테스트의 종류와 이유, 철학을 Java, SpringBoot, JUnit을 사용한 예시와 함께 설명한다.
...
## 22. [🙉FixtureMonkey로 TestFixture를 만들고 테스트 코드 작성하기
...
## 23. [OpenAI의 새로운 Structured Outputs 기능으로 GPT API의  원하는 데이터 형식의 출력을 완벽하게 제어하세요. JSON 스키마 기반의 정확한 데이터 구조화로 AI 애플리케이션 개발의 새 지평을 엽니다.](https://search.app/DmoBcX9wZETHtxto8)
> 생성: 2025-01-07
...
## 24. [구글 개발자 계정 확인 기한 '조직' 계정 인증 | 스윙투앱 도움말](https://documentation.swing2app.co.kr/knowledgebase/playstore/organization)
> 생성: 2025-05-19
