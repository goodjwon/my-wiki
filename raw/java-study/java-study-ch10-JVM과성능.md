...
- `= null`은 만능 해결책이 아니라, 실제 참조 구조를 이해하는 것이 먼저입니다.

<!-- 복구 노트: 2026-07-04 Notion 재수집으로 원본 개정분 병합 (자주 헷갈리는 지점 후반 3개 불릿, 공식 문서, 정리, 한 줄 정리 — 재수집 덤프에는 링크 URL이 유실되어 문서명만 보존) -->
...
<!-- 복구 노트: 2026-07-04 Notion 재수집으로 원본 개정분 병합 (7.3 마지막 불릿, 8~11절, 공식 문서, 정리, 한 줄 정리 — 재수집 덤프에는 링크 URL이 유실되어 문서명만 보존) -->
...
- GC 후에도 사용량이 다시 낮아지는지 본다.
- `GC.class_histogram`으로 어떤 객체가 많은지 본다.
- 캐시, static 컬렉션, 장수 객체 참조를 의심한다.
- 필요하면 힙 덤프를 남긴다.
##### 8.2 pause가 너무 길다

먼저 해야 할 일:

- GC 로그로 pause가 실제 원인인지 확인한다.
- 힙이 너무 작아 수집이 과도하게 자주 일어나는지 본다.
- 너무 큰 객체 그래프나 대량 적재 구간이 있는지 본다.
- 그다음에야 GC 관련 옵션 조정을 검토한다.
##### 8.3 `OutOfMemoryError: Metaspace`

먼저 해야 할 일:

- 클래스 수가 비정상적으로 늘어나는지 본다.
- 프록시 생성, 리로딩, 클래스 로더 누수를 의심한다.
- 단순히 힙을 늘리는 것으로 해결하려 하지 않는다.
##### 8.4 CPU는 낮은데 처리량이 안 나온다

먼저 해야 할 일:

- 스레드 덤프를 본다.
- 락 경합이나 외부 I/O 대기를 의심한다.
- GC가 아니라 스레드 상태가 병목일 가능성을 확인한다.
#### 9. JFR은 "나중 고급 도구"가 아니라 실전 기본기다

성능 이슈를 한 번에 넓게 보려면 Java Flight Recorder를 익히는 것이 좋습니다. 특히 GC, 스레드, 할당, 메서드 hotspot을 한 흐름으로 볼 수 있다는 점이 강합니다.
...
아래 설정은 만능값이 아니라, 관찰 가능한 상태를 만드는 출발점 정도로 보는 편이 맞습니다.
...
현재 저장소에 바로 적용된 값은 아니고, 실제로는 아래처럼 더 보수적인 컨테이너 기본값이 먼저 잡혀 있습니다.

```bash
java -XX:+UseContainerSupport -XX:MaxRAMPercentage=75.0 -jar app.jar
```

<!-- 검증 노트: 2026-07-04 UseContainerSupport는 JDK 10+ 리눅스에서 기본 활성(default true, JDK 21 java 매뉴얼 확인)이므로 +UseContainerSupport 명시는 동작 변경이 아니라 의도 문서화에 해당. 반면 MaxRAMPercentage 기본값은 25.0이므로 75.0 지정은 실제 효력 있는 조정. -->

```plain text
예상 결과
운영용 기본 템플릿은 장애 분석 자료를 남기기 위한 확장 예시다.
현재 프로젝트는 이보다 단순한 기본값으로 시작하고 있으며, GC 로그나 힙 덤프 설정은 필요 시 운영 환경에서 추가하는 방식으로 읽는 편이 정확하다.
```
